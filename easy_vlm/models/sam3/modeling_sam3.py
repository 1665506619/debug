...
class Sam3DetrDecoder(Sam3PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        """
        query_embed = kwargs.pop('query_embed', None)
...
if __name__=='__main__':
    ...
    from PIL import Image   
    device = 'cuda'
    model = Sam3Model.from_pretrained("/mnt/workspace/workgroup/yuanyq/checkpoints/sam3").to(device)
    processor = Sam3Processor.from_pretrained("/mnt/workspace/workgroup/yuanyq/checkpoints/sam3")

    image = Image.open('/mnt/workspace/workgroup/yuanyq/code/LISA/imgs/blackpink.jpg').convert("RGB")
    images = [
        image,image,image
    ]

    inputs = processor(images=images, text=["ear","dial","cat"], return_tensors="pt").to(device)
    import pdb
    pdb.set_trace()
    with torch.no_grad():
        outputs = model(**inputs)