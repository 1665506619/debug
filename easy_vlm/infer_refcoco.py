...
from easy_vlm import disable_torch_init
...
def main():
    ...
    data = json.load(open('/mnt/workspace/workgroup/yuanyq/code/LISA/utils/new_format/refcoco.json'))
    model_path = "/mnt/workspace/workgroup/yuanyq/checkpoints/1224_pretrain_v1_10_query_lora"
    ...
    os.makedirs("vis_refcoco", exist_ok=True)

    for idx, d in enumerate(tqdm(data)):
        try:
            image_file = os.path.join('/mnt/workspace/workgroup/yuanyq/video_data/coco/train2017/', d['image'].split('/')[-1].split('_')[-1])
        ...
        for i,cat in enumerate(d['cat']):
            instruction = f"Please segment '{cat}' the image. "
            contents = []
            ...
            gt_mask = annToMask(d['masks'][i], h=height, w=width)  # ndarray, HxW (0/1)
            ...
            save_path = os.path.join(
                "vis_refcoco",
                f"{d['image'].split('/')[-1].replace('.jpg', '')}_{cat}.png"
            )
