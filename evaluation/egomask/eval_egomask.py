...
from evaluation.egomask.metrics import db_eval_iou, db_eval_boundary, db_eval_boundary_temporal
import multiprocessing as mp
from evaluation.egomask.common_utils import *
...
def eval_queue(q, rank, out_dict, mask_dir, pred_path):
    while not q.empty():
        ...
        if not os.path.exists(f"{pred_path}/{vid_name}"):
            print(f"Prediction for {pred_path}/{vid_name} not found, skipping...")
            continue
...
if __name__ == "__main__":
    ...
    if cnt != len(output_dict):
        print(f"警告: 期望 {cnt} 个样本，实际处理 {len(output_dict)} 个样本（缺失 {cnt - len(output_dict)} 个）")
        print("继续评估已有的样本...")
    # assert cnt == len(output_dict)  # 注释掉，允许部分样本缺失
